...
// example_lights.dart), lightly adapted: a grid of animated colored point
// lights over a grid of shapes, rendered with the declarative SceneView
// widget instead of a hand-rolled ticker + CustomPainter.
...
/// reaches only nearby shapes, so per-object culling keeps every fragment
/// cheap while the scene as a whole holds an unlimited number of lights.
class SceneLightsDemo extends StatefulWidget {
  const SceneLightsDemo({super.key});

  @override
  State<SceneLightsDemo> createState() => _SceneLightsDemoState();
}

/// Bobs the owning node up and down about a base position, so its light
/// drifts and the culling (and light pools) update every frame.
...
class _SceneLightsDemoState extends State<SceneLightsDemo> {
  final Scene scene = Scene();

  @override
  void initState() {
    // Dim the image-based ambient and drop the sun so the colored point
    // lights are what the eye reads. Reflect the lit scene off the floor.
    scene.environmentIntensity = 0.1;
    scene.directionalLight = null;
    scene.screenSpaceReflections.enabled = true;

    const double extent = (_grid - 1) * _spacing / 2;

    int index = 0;
    for (int i = 0; i < _grid; i++) {
      for (int j = 0; j < _grid; j++) {
        final double x = i * _spacing - extent;
        final double z = j * _spacing - extent;
        final double t = index / (_grid * _grid);

        // The floor is tiled, one dark near-polished tile per cell, rather
        // than one large slab: per-object culling gives each object the
        // lights that reach it, so a single floor spanning every light would
        // be reached by all of them at once (and exceed the per-object
        // budget). Tiling keeps each piece within reach of only its local
        // lights.
        scene.add(
          Node(
            mesh: Mesh(
        ...
        // A shape at the grid cell, its material sweeping metal/dielectric
        // and roughness across the grid.
        scene.add(
          Node(
            mesh: Mesh(
  ...
  Widget build(BuildContext context) {
    const double radius = _grid * _spacing * 0.9;
    return SceneView(
      scene,
      cameraBuilder: (Duration elapsed) {
        final double t = elapsed.inMicroseconds / 1e6 * 0.15;
