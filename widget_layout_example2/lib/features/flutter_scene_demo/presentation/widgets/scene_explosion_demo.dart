...
// example_explosion.dart), adapted to this project: the example app's shared
// overlay/panel/settings scaffolding is replaced with plain Material controls
// below the viewport.
...
import 'package:widget_layout_example2/features/flutter_scene_demo/presentation/widgets/vfx_textures.dart';
...
/// chunks that arc away with additive ribbon trails. Controls below the
/// viewport detonate it on demand and tune the cycle.
class SceneExplosionDemo extends StatefulWidget {
  const SceneExplosionDemo({super.key, this.viewportHeight = 300});

  final double viewportHeight;

  @override
  State<SceneExplosionDemo> createState() => _SceneExplosionDemoState();
}
...
class _HeroChunk extends Component {
  ...
  void boom(Random rng) {
    ...
    final double tilt = 0.15 + rng.nextDouble() * 0.5;
    final double speed = 6.0 + rng.nextDouble() * 4.5;
    _velocity.setValues(
...
class _ExplosionDirector extends Component {
  ...
  void detonate() {
    for (final ParticleSystem system in systems) {
      system.reset();
    }
    for (final _HeroChunk chunk in chunks) {
      chunk.boom(_rng);
    }
    _sinceBoom = 0;
  }
  ...
  void update(double deltaSeconds) {
    ...
    final double t = (_sinceBoom / 0.55).clamp(0.0, 1.0);
    final double eased = 1.0 - (1.0 - t) * (1.0 - t);
    final double radius = 0.4 + 7.2 * eased;
...
class _SceneExplosionDemoState extends State<SceneExplosionDemo> {
  ...
  Future<void> _load() async {
    ...
    final GpuTextureSource fireballAtlas = GpuTextureSource(
      await gpuTextureFromImage(await bakeFireballAtlas()),
    );
    final GpuTextureSource smokeAtlas = GpuTextureSource(
      await gpuTextureFromImage(await bakeSmokeAtlas()),
    );
    final GpuTextureSource dot = GpuTextureSource(
      await gpuTextureFromImage(await bakeSoftDot()),
    );

    _buildSet();

    final ParticleSystem fireball = _fireball(fireballAtlas);
    final ParticleSystem sparks = _sparks(dot);
    final ParticleSystem smoke = _smoke(smokeAtlas);
    final ParticleSystem debris = _debris();

    // Shockwave ring, swept by the director.
    final UnlitMaterial shockwaveMaterial = UnlitMaterial()
      ..alphaMode = AlphaMode.blend
      ..baseColorFactor = vm.Vector4(2.6, 1.9, 1.3, 0.0);
    final Node shockwaveNode = Node(
      mesh: Mesh(
    ...
    final CuboidGeometry chunkGeometry = CuboidGeometry(
      vm.Vector3(0.16, 0.11, 0.13),
    );
    final PhysicallyBasedMaterial chunkMaterial = PhysicallyBasedMaterial()
      ..baseColorFactor = vm.Vector4(0.16, 0.14, 0.12, 1)
      ..roughnessFactor = 0.7
      ..metallicFactor = 0.1
      ..emissiveFactor = vm.Vector4(1.3, 0.32, 0.05, 1);
    final List<_HeroChunk> chunks = <_HeroChunk>[];
    for (int i = 0; i < 6; i++) {
      final TrailComponent trail = TrailComponent(
        width: 0.16,
        lifetime: 0.5,
        minVertexDistance: 0.08,
        maxPoints: 40,
        colorOverTrail: ColorGradient(<ColorStop>[
      ...
      final _HeroChunk chunk = _HeroChunk(trail);
      chunks.add(chunk);
      scene.add(
    ...
    _director = _ExplosionDirector(
      systems: <ParticleSystem>[fireball, sparks, smoke, debris],
      chunks: chunks,
  ...
  void _buildSet() {
    ...
    for (int i = 0; i < 5; i++) {
      final double angle = i * 2 * pi / 5 + rng.nextDouble() * 0.5;
      final double r = 3.2 + rng.nextDouble() * 1.6;
      final double w = 0.7 + rng.nextDouble() * 0.9;
      final double h = 0.5 + rng.nextDouble() * 0.9;
      scene.add(
        Node(
          mesh: Mesh(
  ...
  ParticleSystem _debris() {
    final ParticleSystem system = ParticleSystem(
      ...
      gravity: vm.Vector3(0, -9.8, 0),
      modules: const <ParticleModule>[RotationModule()],
      seed: 24,
    );
    ...
    final MeshParticleEmitterComponent emitter = MeshParticleEmitterComponent(
      system: system,
      geometries: <Geometry>[
      ...
      children: <Widget>[
        SizedBox(
          ...
          child: _ready
              ? ClipRect(
                  child: SceneView(
                    scene,
                    cameraBuilder: (Duration elapsed) {
        ...
        if (_ready) ...<Widget>[
          ...
          SwitchListTile(
            ...
            title: const Text('Auto detonate'),
            subtitle: const Text('Re-triggers the cycle on an interval'),
            value: _director.auto,
            onChanged: (bool value) => setState(() => _director.auto = value),
          ),
          Row(
            children: <Widget>[
              const SizedBox(width: 190, child: Text('Interval (seconds)')),
              Expanded(
                child: Slider(
                  ...
                  onChanged: (double value) =>
                      setState(() => _director.interval = value),
                ),
              ),
              SizedBox(
                width: 40,
                child: Text(_director.interval.toStringAsFixed(1)),
              ),
            ],
          ),
        ],
