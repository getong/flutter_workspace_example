// Adapted from the flutter_scene example app's main.dart
// (examples/flutter_app/lib/main.dart): the same gallery of examples with the
// shared post-processing settings sidebar, repackaged as a routed page
// instead of a standalone MaterialApp. Examples requiring native physics
// backends (Rapier, box3d), the SoLoud audio backend, or the multiplayer
// stack are omitted.
...
/// Builders for every example in the gallery, in display order. Exposed so
/// the flutter_scene feature page can list the catalog and deep-link into
/// any example.
final Map<String, WidgetBuilder> galleryExamples = {
...
/// The flutter_scene example gallery: an example picker (top-left), the
/// shared settings sidebar (top-right), and one full-screen example at a
/// time.
class SceneGalleryPage extends StatefulWidget {
  const SceneGalleryPage({super.key, this.initialExample});

  /// The example to open with; defaults to the first catalog entry.
  final String? initialExample;

  @override
  State<SceneGalleryPage> createState() => _SceneGalleryPageState();
}

class _SceneGalleryPageState extends State<SceneGalleryPage> {
  ...
  Map<String, WidgetBuilder> get examples => galleryExamples;

  @override
  void initState() {
    selectedExample = examples.containsKey(widget.initialExample)
        ...
        builder: (context, snapshot) {
          ...
          return Stack(
            children: [
              ...
              // Back button + example picker (top-left, overlaid on the
              // scene).
              SafeArea(
                      ...
                      _ExamplePicker(
                        ...
                        onSelected: (next) {
                          setState(() {
                            ...
                            // Every example runs on its own settings
                            // instance; examples listed in settingsDefaults
                            // start from their own defaults instead of the
                            // stock ones.
