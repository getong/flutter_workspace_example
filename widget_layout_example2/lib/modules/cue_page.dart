...
class _CuePageState extends State<CuePage> {
  ...
  _workflows = <({String title, String summary, IconData icon, Color color})>[
    (
      ...
      summary: 'Stagger a title, subtitle, and chips when the section mounts with smooth physics.',
      icon: Icons.rocket_launch,
      color: Color(0xFF2A6F97),
    ),
    (
      title: 'Toggle Panels',
      summary: 'Drive expansion and reversal from a single boolean state flag with coordinated animations.',
      icon: Icons.expand_circle_down,
      color: Color(0xFF7B2CBF),
    ),
    (
      title: 'Scroll Reactions',
      summary: 'Scrub or reveal cards as they move through the viewport with scroll-based triggers.',
        ...
        padding: const EdgeInsets.all(24),
          children: <Widget>[
                    ...
                    child: Row(
                      children: List<Widget>.generate(
                        ...
                        (int index) => Padding(
                          ...
                          child: ChoiceChip(
                            label: Text(<String>['Overview', 'Details', 'Advanced'][index]),
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        _ActDefinition(
                          ...
                          description: 'Controls opacity. Perfect for entrance and exit transitions.',
                        ),
                        const SizedBox(height: 12),
                        _ActDefinition(
                          name: 'slideX / slideY',
                          example: '.slideX(from: -0.3)  or  .slideY(to: 0.2)',
                          description: 'Moves widget horizontally or vertically as percentage of viewport.',
                        ),
                        const SizedBox(height: 12),
                        _ActDefinition(
                          name: 'scale',
                          example: '.scale(from: 0.8)  or  .scale(to: 1.2)',
                          description: 'Resizes widget from origin point. 1.0 is normal size.',
                        ),
                        const SizedBox(height: 12),
                        _ActDefinition(
                          name: 'rotate',
                          example: '.rotate(from: -180)  or  .rotate(to: 360)',
                          description: 'Rotates widget in degrees. Perfect for loading spinners.',
                        ),
                        const SizedBox(height: 12),
                        _ActDefinition(
                          name: 'blur',
                          example: '.blur(from: 8)  or  .blur(to: 0)',
                          description: 'Applies Gaussian blur filter. Great for focus effects.',
                        ),
                        const SizedBox(height: 12),
                        _ActDefinition(
                          name: 'clipHeight / clipWidth',
                          example: '.clipHeight()  or  .clipWidth()',
                          description: 'Reveals/hides content by clipping. Best for expand/collapse.',
                        ),
                      ],
                    ).act(<Act>[
                      .fadeIn(),
            ...
            _CueSectionCard(
              ...
              codeLabel: 'for (item in items) Actor(delay: i * 80.ms, acts: [...], child: item)',
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  FilledButton.tonalIcon(
                    ...
                    label: Text(_showListItems ? 'Hide List' : 'Show Staggered List'),
                  ),
                  const SizedBox(height: 12),
                  if (_showListItems)
                    Cue.onMount(
            ...
            _CueSectionCard(
              ...
              codeLabel: 'Cue.onToggle(toggled: isValid, acts: [.scale(from: 0.8), .fadeIn()])',
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  FilledButton.icon(
            ...
            _CueSectionCard(
              ...
              codeLabel: 'Cue.onToggle(toggled: isSelected, acts: [.rotate(to: 0.5), .scale(...)])',
              child: Column(
                children: List<Widget>.generate(3, (int index) {
            ...
            _CueSectionCard(
              ...
              codeLabel: '.smooth()  .easeInOut()  .easeOut()  .easeIn()  .linear()',
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Cue.onMount(
                    ...
                    child: Column(
                      children: <Widget>[
                        _MotionCurveExample(
                          ...
                          description: 'Natural, bouncy motion. Best for user interactions.',
                          curve: 'smooth',
                        ),
                        const SizedBox(height: 12),
                        _MotionCurveExample(
                          title: '.easeInOut() - Smooth Acceleration',
                          description: 'Starts slow, speeds up mid, slows at end.',
                          curve: 'easeInOut',
                        ),
                        const SizedBox(height: 12),
                        _MotionCurveExample(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        _UseCaseItem(
                          ...
                          description: 'Entrance animations + hover scale effects for product grids',
                        ),
                        const SizedBox(height: 12),
                        _UseCaseItem(
                          icon: Icons.chat,
                          title: 'Chat Message Bubbles',
                          description: 'Sequential message animations with staggered delays',
                        ),
                        const SizedBox(height: 12),
                        _UseCaseItem(
                          icon: Icons.dashboard,
                          title: 'Dashboard Metrics',
                          description: 'Scroll-triggered animations for KPI cards',
                        ),
                        const SizedBox(height: 12),
                        _UseCaseItem(
                          icon: Icons.task_alt,
                          title: 'Todo List Items',
                          description: 'Completion animations with checkmark rotations',
                        ),
                        const SizedBox(height: 12),
                        _UseCaseItem(
                          icon: Icons.notifications,
                          title: 'Notification Toasts',
                          description: 'Slide-in entrance and fade-out exit animations',
                        ),
                        const SizedBox(height: 12),
                        _UseCaseItem(
                          icon: Icons.menu,
                          title: 'Navigation Drawers',
                          description: 'Expandable menu items with reveal animations',
                        ),
                      ],
                    ).act(<Act>[
                      .fadeIn(),
            ...
            _CueSectionCard(
              ...
              description: 'Guidelines for using Cue effectively and efficiently in production apps.',
              codeLabel: '',
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
